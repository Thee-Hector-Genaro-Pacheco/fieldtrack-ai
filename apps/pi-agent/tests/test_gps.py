...
def test_valid_live_gps_data():
    state = GPSState(mode="auto")
    state.serial_connected = True
    # Live Irvine NMEA GGA sentence
    sentence = make_sentence("GPGGA,123519,3340.1580,N,11751.3780,W,1,09,0.8,42.0,M,0.0,M,,")
    success = state.update_from_nmea(sentence)
    assert success is True
    
    tel = state.get_telemetry_snapshot(public_demo_mode=False, stale_after_seconds=5)
    assert tel.source == "live"
    assert tel.device_health.gps_mode == "live"
    assert tel.fix is True
    assert pytest.approx(tel.latitude, 0.0001) == 33.6693
    assert pytest.approx(tel.longitude, 0.0001) == -117.8563
    assert tel.satellites_used == 9
    assert tel.hdop == 0.8


def test_no_fix_gps_data():
    state = GPSState(mode="auto")
    state.serial_connected = True
    # GGA sentence with fix quality 0 (no fix)
    sentence = make_sentence("GPGGA,123519,3340.1580,N,11751.3780,W,0,04,2.5,42.0,M,0.0,M,,")
    success = state.update_from_nmea(sentence)
    assert success is True
    
    tel = state.get_telemetry_snapshot(public_demo_mode=False, stale_after_seconds=5)
    assert tel.source == "live"
    assert tel.fix is False


def test_serial_unavailable():
    state = GPSState(mode="auto")
    state.serial_connected = False
    
    tel = state.get_telemetry_snapshot(public_demo_mode=False, stale_after_seconds=5)
    assert tel.source == "mock"
    assert tel.device_health.gps_mode == "mock"
    assert tel.device_health.status == "unhealthy"
    assert "Serial port" in (tel.device_health.last_error or "")
...
def test_recovery_from_mock_mode_to_live():
    state = GPSState(mode="auto")
    state.serial_connected = False
    
    # Initial state: mock fallback
    tel_initial = state.get_telemetry_snapshot(public_demo_mode=False, stale_after_seconds=5)
    assert tel_initial.source == "mock"
    
    # Recovery: Serial connects and receives valid sentence
    state.serial_connected = True
    sentence = make_sentence("GPGGA,123519,3340.1580,N,11751.3780,W,1,09,0.8,42.0,M,0.0,M,,")
    state.update_from_nmea(sentence)
    
    # Recovered state: live source
    tel_recovered = state.get_telemetry_snapshot(public_demo_mode=False, stale_after_seconds=5)
    assert tel_recovered.source == "live"
    assert tel_recovered.device_health.gps_mode == "live"
    assert tel_recovered.fix is True
...
def test_stale_telemetry_detection():
    ...
    assert tel_stale.fix is False
    assert tel_stale.source == "mock"  # Stale data triggers mock fallback
